...
let rec collect_box_exp (e: gen_exp): (int * g_type) list = match e with
	...
	| TagTupleExpC(i, tau, _, el) -> (i, tau) :: (List.concat (List.map collect_box_exp el))
	| TupleIndexExpC(e, _, _) -> collect_box_exp e
	| _ -> []

let rec collect_box_stmt (s: gen_stmt): (int * g_type) list = match s with
	...
	| GCCollectStmtC -> []
and collect_box_body (b: gen_stmt list): (int * g_type) list = match b with
	[] -> []
