...
type tc_err =
	...
	| BadReturn_Err of g_type * g_type * l_pos
		(* constant evaluation *)
...
let string_of_first_arg (t: g_type option): string = match t with
	None -> "empty arg list"
	| Some tau -> "type \"" ^ (string_of_type cr tau) ^ "\""

let string_of_tc_err (e: tc_err) = match e with
	| BadType_Err(s, t, p) -> "Expected type \"" ^ (string_of_type cr t) ^ "\" and received type \"" ^
		(string_of_type cr s) ^ "\" at " ^ (string_of_pos p) ^ "."
	...
	| NonStruct_Err(t, p) -> "Struct operation called on non-struct type \"" ^ (string_of_type cr t) ^ "\" at " ^ (string_of_pos p) ^ "."
	...
	| BadReturn_Err(s, t, p) ->
		"Function expected return type \"" ^ (string_of_type cr t) ^ "\" and received value of type \"" ^
			(string_of_type cr s) ^ "\" at " ^ (string_of_pos p) ^ "."
	| BadCLoad_Err(f, p) ->
